...
export function SlidePanel({ isOpen, onClose, title, children }: SlidePanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-stone-900 border-l border-stone-800 shadow-2xl z-50
          flex flex-col transform transition-transform duration-300 ease-out
          ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-800 shrink-0">
          <h2 className="font-semibold text-stone-100 text-base tracking-tight">{title}</h2>
          <button
            onClick={onClose}
