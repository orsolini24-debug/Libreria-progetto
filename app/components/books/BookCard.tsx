...
export function BookCard({ book, onClick }: { book: Book; onClick: (b: Book) => void }) {
  ...
  const percentage = (book.currentPage != null && book.pageCount) 
  ...
  return (
    ...
    >
      ...
      <div className="glass px-2.5 py-2.5 rounded-b-xl">
        ...
        {/* #70: Progress bar con Tooltip */}
        {book.status === "READING" && percentage !== null && (
          <div className="mt-2 px-0.5" title={`${book.currentPage}/${book.pageCount} pagine (${percentage}%)`}>
            ...
            <p className="text-[9px] mt-0.5 text-right opacity-60" style={{ color: "var(--fg-subtle)" }}>
              {percentage}%
            </p>
          </div>
        )}
